...
void Bitmap::drawText(const IntRect &rect, const char *str, int align)
{
    ...
    SDL_Surface *txtSurf = nullptr;

    if (shState->config().cosmosRGSS301Text && CosmosGDIText::available())
        txtSurf = CosmosGDIText::renderText(sdlFont, *p->font, str, c, p->font->isSolid());

    /* Fallback automatique : renderer mkxp-z original. */
    if (!txtSurf)
    {
        if (p->font->isSolid())
    ...
    if (scaledOutlineSize)
    {
        ...
        if (shState->config().cosmosRGSS301Text && CosmosGDIText::available())
            outline = CosmosGDIText::renderOutline(sdlFont, *p->font, str, co,
                                                   p->font->isSolid(), scaledOutlineSize);

        /* Fallback automatique : outline SDL_ttf original. */
        if (!outline)
        {
            if (p->font->isSolid())
...
IntRect Bitmap::textSize(const char *str)
{
    ...
    if (shState->config().cosmosRGSS301Text && CosmosGDIText::available())
    {
        int cosmosW = 0;
        int cosmosH = 0;
        if (CosmosGDIText::measureText(sdlFont, *p->font, str, cosmosW, cosmosH))
            return IntRect(0, 0, cosmosW, cosmosH);
    }
